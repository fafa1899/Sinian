...
namespace Sinian
{
	...
	public:
		...
		inline const std::map<std::string, std::shared_ptr<Object3D>>& Children() const { return children; }
		...
		const glm::mat4 &WorldModelMatrixInverse() const { return worldModelMatrixInverse; }

		//LocalModel
		void LocalModelTranform(const glm::vec3& position, const glm::vec3& euler);
		void LocalModelTranform(const glm::vec3& position, const glm::vec3& euler, const glm::vec3& scale);

		void LookAt(const glm::vec3& eye, const glm::vec3& center, glm::vec3 worldUp = glm::vec3(0.0f, 1.0f, 0.0f));
