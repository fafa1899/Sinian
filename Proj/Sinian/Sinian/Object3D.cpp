...
void Object3D::UpdateWorldFromParent() {
  ...
  worldModelMatrixInverse =
      localModelMatrixInverse * parent->WorldModelMatrixInverse();

  glm::quat quaternion;
  glm::vec3 skew;
  glm::vec4 perspective;
  glm::decompose(worldModelMatrix, worldScale, quaternion, worldPosition, skew,
                 perspective);

  //
  worldTranslationMatrix =
      glm::translate(glm::identity<glm::mat4>(), worldPosition);
  worldTranslationMatrixInverse =
      glm::translate(glm::identity<glm::mat4>(), -worldPosition);

  //
  worldRotationMatrix = glm::toMat4(quaternion);
  worldRotationMatrixInverse = glm::transpose(worldRotationMatrix);
  glm::extractEulerAngleYXZ(worldRotationMatrix, worldEulerAngles.y,
                            worldEulerAngles.x, worldEulerAngles.z);
  ...
  worldScaleMatrixInverse =
      glm::scale(glm::identity<glm::mat4>(), 1.0f / worldScale);
}

void Object3D::UpdateLocalFromParent() {
  localModelMatrix = parent->WorldModelMatrixInverse() * worldModelMatrix;
  localModelMatrixInverse =
      worldModelMatrixInverse * parent->WorldModelMatrix();

  glm::quat quaternion;
  glm::vec3 skew;
  glm::vec4 perspective;
  glm::decompose(localModelMatrix, localScale, quaternion, localPosition, skew,
                 perspective);

  //
  localTranslationMatrix =
      glm::translate(glm::identity<glm::mat4>(), localPosition);
  localTranslationMatrixInverse =
      glm::translate(glm::identity<glm::mat4>(), -localPosition);

  //
  localRotationMatrix = glm::toMat4(quaternion);
  localRotationMatrixInverse = glm::transpose(localRotationMatrix);
  glm::extractEulerAngleYXZ(localRotationMatrix, localEulerAngles.y,
                            localEulerAngles.x, localEulerAngles.z);

  //
  localScaleMatrix = glm::scale(glm::identity<glm::mat4>(), localScale);
  localScaleMatrixInverse =
      glm::scale(glm::identity<glm::mat4>(), 1.0f / localScale);
}

void Object3D::UpdateLocalModelFromTRS() {
  localModelMatrix =
      localTranslationMatrix * localRotationMatrix * localScaleMatrix;
  localModelMatrixInverse = localScaleMatrixInverse *
                            localRotationMatrixInverse *
                            localTranslationMatrixInverse;
}

void Object3D::UpdateWorldModelFromTRS() {
  worldModelMatrix =
      worldTranslationMatrix * worldRotationMatrix * worldScaleMatrix;
  worldModelMatrixInverse = worldScaleMatrixInverse *
                            worldRotationMatrixInverse *
                            worldTranslationMatrixInverse;
}
...
void Object3D::WorldModelTranform(const glm::vec3& position,
...
void Object3D::LocalModelTranform(const glm::vec3& position,
                                  const glm::vec3& euler) {
  localPosition = position;
  localTranslationMatrix =
      glm::translate(glm::identity<glm::mat4>(), localPosition);
...
void Object3D::LookAt(const glm::vec3& eye, const glm::vec3& center,
                      glm::vec3 worldUp) {
  ...
  // glm::extractEulerAngleZXY(worldModelMatrix, worldEulerAngles.z,
  // worldEulerAngles.x, worldEulerAngles.y); cout <<
  // glm::degrees(worldEulerAngles.z) << '\t' <<
  // glm::degrees(worldEulerAngles.x)
  // << '\t' << glm::degrees(worldEulerAngles.y) << endl;
